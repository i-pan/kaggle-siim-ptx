...
class LovaszHinge(nn.Module):
    ...
    #
    def forward(self, probas, labels, per_image=False, ignore=None):
        probas = probas[:,1,...]
        return (lovasz_hinge(probas, labels, per_image, ignore) + lovasz_hinge(-probas, 1-labels, per_image, ignore))/2
...
class LovaszSoftmax(nn.Module):
    ...
    #
    def forward(self, probas, labels, only_present=False, per_image=False, ignore=None):
        probas = torch.softmax(probas, dim=1)
        assert probas.min() >= 0 and probas.max() <= 1
        return lovasz_softmax(probas, labels, only_present, per_image, ignore)
