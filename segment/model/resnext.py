...
class ResNeXt(ResNet):
    ...
    def __init__(self, groups=1, base_width=4, **kwargs):
        ...
        for i, num_blocks in enumerate(self.stage_blocks):
            ...
            planes = 64 * 2**i
            # Allow for DCN + multigrid final block
            if i == len(self.stage_blocks) - 1: #and not True in self.stage_with_dcn:
                assert len(self.mg_rates) == num_blocks
                res_layer = make_multigrid(
                    self.block,
                    self.inplanes,
                    planes,
                    self.mg_rates,
